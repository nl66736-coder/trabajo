...
def test_menu_tiene_perfil():
    ...
    if '"Mi Perfil"' in contenido and '"/perfil"' in contenido:
        ...
        print("⚠️  Menú hace referencia a perfil (revisar texto exacto)")
        return True
    else:
...
def test_estructura_perfiles_json():
  ...
  """)
    ...
    if os.path.exists("perfiles.json"):
        ...
        print("ℹ️  perfiles.json no existe aún (se creará automáticamente)")
...
def run_all_tests():
    ...
    if exitos == total:
        ...
        print(f"\n⚠️  {total - exitos} test(s) fallaron. Revisa los errores.")
